...
/**
 * Staff Book Repository
 * =========================================================================
 * Handles inventory management and advanced book search for staff members.
 * Includes native queries for genre management and JPQL for multi-field search.
 */
@Repository
public interface StaffBookRepository extends JpaRepository<Book, Integer> {
	...
	@Query("SELECT b FROM Book b WHERE b.title LIKE CONCAT('%', :q, '%') OR b.isbn LIKE CONCAT(:q, '%') OR b.author LIKE CONCAT('%', :q, '%') OR b.publisher LIKE CONCAT('%', :q, '%')")
